...
LRESULT WindowImplBase::OnNcCalcSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if ( ::IsZoomed(m_hWnd))
	{	// 最大化时，计算当前显示器最适合宽高度, 此处稍坐修改
#if 1   
		//by zhaohaibo   此段代码会导致最大化的时候ResizeWindow出现问题
		MONITORINFO oMonitor = {};
		oMonitor.cbSize = sizeof(oMonitor);
		::GetMonitorInfo(::MonitorFromWindow(*this, MONITOR_DEFAULTTONEAREST), &oMonitor);
		CDuiRect rcWork = oMonitor.rcWork;
		CDuiRect rcMonitor = oMonitor.rcMonitor;
		if ((pRect->right-pRect->left)<(rcWork.right-rcWork.left) || (pRect->bottom - pRect->top)<(rcWork.bottom-rcWork.top))
		{
			return 0;
		}
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	CDuiString strResourcePath= m_PaintManager.GetResourcePath(); //m_PaintManager.GetInstancePath();
