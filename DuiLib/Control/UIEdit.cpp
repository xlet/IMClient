...
namespace DuiLib
{
	...
	void CEditWnd::Init(CEditUI* pOwner)
	{
		m_pOwner = pOwner;
		RECT rcPos = CalPos();
		/////modified by zhaohaibo 201410162200
		////old
		//UINT uStyle = WS_CHILD | ES_AUTOHSCROLL; 
		//new code
		UINT uStyle = WS_CHILD ;
		if (m_pOwner->GetAutoHScorll())
		{
	...
	//
	//
	//add by zhaohaibo 201410162044

// 	void CEditUI::SetAutoHScroll(bool b/* =true */)
// 	{
// 		DWORD dwStyle = ::GetWindowLong(m_pWindow->GetHWND(), GWL_STYLE);  
// 		dwStyle &= ~ES_AUTOHSCROLL;
// 		::SetWindowLong(m_pWindow->GetHWND(), GWL_STYLE, dwStyle);
// 		
// 	}

	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		m_bPasswordMode(false), m_cPasswordChar(_T('*')), m_uButtonState(0), 
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0),m_bAutohScroll(true)
	{
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		//add by zhaohaib0 201410162150
		//增加水平滚动属性
		else if (_tcscmp(pstrName, _T("atuohscroll")) == 0)
		{
			SetAutoHScroll(_tcscmp(pstrValue, _T("true")) == 0);
		}
		//end
		else CLabelUI::SetAttribute(pstrName, pstrValue);
	}

	//add by zhaohaibo 201410162153
	void CEditUI::SetAutoHScroll(bool b)
	{
		m_bAutohScroll=b;
	}
	bool CEditUI::GetAutoHScorll()
	{
