...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
		...
		//modified by　赵海波
		//边缘一像素， item和容器间的间隙一像素 inset 设置为2，2,2,2
		//注意Item布局中如果用pading , 由于Combownd容器本身用了Inset  可能产生布局bug, 即控件位置产生误差
	   ...
	   //end 
        ...
        m_pOwner->SetFocus();
		m_pOwner->GetManager()->SendNotify(m_pOwner, _T("dropwndhidden"));
    }
...
CWindowWnd* CComboUI::GetComboWnd()
{
	return m_pWindow;
}
