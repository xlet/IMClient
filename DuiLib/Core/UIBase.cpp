...
void CWindowWnd::ResizeWindow(int cx/* = -1 */, int cy /* = -1 */)
{
	ASSERT(::IsWindow(m_hWnd));
	RECT rc = { 0 };
	if( !::GetClientRect(m_hWnd, &rc) ) return;
	if( cx != -1 ) rc.right = cx;
	if( cy != -1 ) rc.bottom = cy;
	int Sceen_X= ::GetSystemMetrics(SM_CXFULLSCREEN);
	int width  = cx>=Sceen_X?Sceen_X:cx;
	//if( !::AdjustWindowRectEx(&rc, GetWindowStyle(m_hWnd), (!(GetWindowStyle(m_hWnd) & WS_CHILD) && (::GetMenu(m_hWnd) != NULL)), GetWindowExStyle(m_hWnd)) ) return;
	
	::SetWindowPos(m_hWnd, NULL, 0, 0, width /*rc.right - rc.left*/, rc.bottom - rc.top, SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
