...
bool CPaintManagerUI::EnableDropTarget(void)
{
	if( m_pDragDrop == 0 )
	{
		if( m_pDragDrop = new IDropTargetImpl(m_hWndPaint) )
		{
			HRESULT hr = S_FALSE;
		hr = 		::RegisterDragDrop(m_hWndPaint,m_pDragDrop);
			return true;
		}
		else
			return false;
...
void CPaintManagerUI::RegisterUI( CControlUI* pUI )
{
	if( m_pDragDrop )
		m_pDragDrop->Register(pUI);
}
...
void CPaintManagerUI::Init(HWND hWnd)
{
	...
	LONG lWndExt = ::GetWindowLong(hWnd,GWL_EXSTYLE);
	if( (lWndExt&WS_EX_ACCEPTFILES) == WS_EX_ACCEPTFILES )
	{
		m_pDragDrop = new IDropTargetImpl(hWnd);
		::RegisterDragDrop(hWnd,m_pDragDrop);
	}
